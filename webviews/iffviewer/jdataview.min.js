...
this._offset=0;if(this._isDataView){for(var dataType in dataTypes){if(!dataTypes.hasOwnProperty(dataType)){continue}(function(type,view){var size=dataTypes[type];view['get'+type]=function(byteOffset,littleEndian){if(littleEndian===undefined){littleEndian=view._littleEndian}
if(byteOffset===undefined){byteOffset=view._offset}
view._offset=byteOffset+size;return view._view['get'+type](byteOffset,littleEndian)}})(dataType,this)}}else if(this._isNodeBuffer&&compatibility.NodeBuffer){for(dataType in dataTypes){if(!dataTypes.hasOwnProperty(dataType)){continue}
var name;if(dataType==='Int8'||dataType==='Uint8'){name='read'+nodeNaming[dataType]}else if(littleEndian){name='read'+nodeNaming[dataType]+'LE'}else{name='read'+nodeNaming[dataType]+'BE'}(function(type,view,name){var size=dataTypes[type];view['get'+type]=function(byteOffset,littleEndian){if(littleEndian===undefined){littleEndian=view._littleEndian}
if(byteOffset===undefined){byteOffset=view._offset}
view._offset=byteOffset+size;return view.buffer[name](view._start+byteOffset)}})(dataType,this,name)}}else{for(dataType in dataTypes){if(!dataTypes.hasOwnProperty(dataType)){continue}(function(type,view){var size=dataTypes[type];view['get'+type]=function(byteOffset,littleEndian){if(littleEndian===undefined){littleEndian=view._littleEndian}
if(byteOffset===undefined){byteOffset=view._offset}
...
return view['_get'+type](view._start+byteOffset,littleEndian)}}})(dataType,this)}}};if(compatibility.NodeBuffer){jDataView.createBuffer=function(){var buffer=Buffer.alloc(arguments.length);for(var i=0;i<arguments.length;++i){buffer[i]=arguments[i]}
