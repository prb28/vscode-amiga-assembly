...
function getEditsForLine(line: string, enableTabs = false): TextEdit[] {
    ...
    return f.computeEditsForLine(asmDocument, asmDocument.asmLinesArray[0]);
}
...
describe("Formatter Tests", function () {
    ...
    it("Should format an assignment", function () {
        ...
        asmDocument.parse(document, conf);
        let edits = f.computeEditsForLine(asmDocument, asmDocument.asmLinesArray[0]);
        let i = 0;
        expect(edits.length).to.be.equal(3);
        ...
        edits = f.computeEditsForLine(asmDocument, asmDocument.asmLinesArray[1]);
        i = 0;
        expect(edits.length).to.be.equal(2);
        ...
        edits = f.computeEditsForLine(asmDocument, asmDocument.asmLinesArray[2]);
        i = 0;
        expect(edits.length).to.be.equal(2);
        ...
        asmDocument.parse(document, conf);
        edits = f.computeEditsForLine(asmDocument, asmDocument.asmLinesArray[0]);
        i = 0;
        expect(edits.length).to.be.equal(3);
        expect(edits[i++]).to.be.eql(TextEdit.replace(new Range(new Position(0, 14), new Position(0, 16)), "\t"));
        expect(edits[i++]).to.be.eql(TextEdit.replace(new Range(new Position(0, 11), new Position(0, 12)), "\t"));
    ...
    it("Should format with preferred sizes", function () {
        ...
        edits = f.computeEditsForLine(asmDocument, asmDocument.asmLinesArray[3]);
        expect(edits.length).to.be.equal(0);
        edits = f.computeEditsForLine(asmDocument, asmDocument.asmLinesArray[4]);
        expect(edits.length).to.be.equal(0);
        edits = f.computeEditsForLine(asmDocument, asmDocument.asmLinesArray[5]);
        expect(edits.length).to.be.equal(0);
        edits = f.computeEditsForLine(asmDocument, asmDocument.asmLinesArray[6]);
        i = 0;
        expect(edits.length).to.be.equal(3);
    ...
    it("Should allow opcode names in labels", function () {
        ...
        const conf = new DocumentFormatterConfiguration(2, 4, 4, 1, 1, 0, 0, false, 4);
        asmDocument.parse(document, conf);
        const edits = f.computeEditsForLine(asmDocument, asmDocument.asmLinesArray[0]);
        expect(edits.length).to.be.equal(0);
    });
});
