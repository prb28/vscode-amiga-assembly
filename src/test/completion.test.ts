...
describe("Completion Tests", function () {
    ...
    describe("Completion api", function () {
        it("Should return no completion on an unknown work", async function () {
            let cp = new M68kCompletionItemProvider(state.getDocumentationManager(), state.getDefinitionHandler());
            const document = new DummyTextDocument();
            let position: Position = new Position(0, 11);
            let tockenEmitter = new CancellationTokenSource();
            document.addLine(" jsr mymymy");
            let results = await cp.provideCompletionItems(document, position, tockenEmitter.token);
        ...
        it("Should not return a completion on an instruction after .", async function () {
            ...
            results = await cp.provideCompletionItems(document, position, tockenEmitter.token);
            expect(results).to.be.empty;
        });
